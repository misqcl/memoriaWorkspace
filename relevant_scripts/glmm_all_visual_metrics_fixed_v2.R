...
# for the four visual metrics used in the thesis.
#
# Models:
...
#
# Input:
#   memoria/heatmap_analysis_clean.xlsx
...
normalize_emotion <- function(x) {
  ...
  case_when(
    ...
    x_chr %in% c("no responde", "no response") ~ "Neutral",
...
fit_glmm <- function(
  ...
  if (length(fitted_models) > 0) {
    warning(
      paste0(
        model_label,
        ": no optimizer produced a positive-definite Hessian. ",
        "Returning the last fitted model; interpret its standard errors ",
        "and pairwise contrasts cautiously."
      )
    )
    return(fitted_models[[length(fitted_models)]])
  }
...
analyze_metric <- function(
  ...
  formulas <- list(
    Null = null_formula,
    `Original emotion` = original_formula,
    `Reported emotion` = reported_formula,
    Combined = combined_formula
  )
    ...
    lapply(
      ...
      function(model_name) {
        extract_model_information(
          ...
          distribution_name = spec$distribution,
          analysis_data = analysis_data
        )
      }
    )
