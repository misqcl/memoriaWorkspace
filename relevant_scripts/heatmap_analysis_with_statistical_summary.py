...
HEATMAP_DIR=BASE_DIR/"image_heatmaps"/"raw"
OUTPUT_FILE=BASE_DIR/"heatmap_analysis_clean.xlsx"
...
def load_responses(users):
    ...
    for _,u in users.iterrows():
        ...
        d=pd.read_csv(p).rename(columns={"Respuesta":"response_value","Emocion":"response_emotion","Archivo":"filename"})
        d["response_emotion"]=d["response_emotion"].apply(normalize_emotion)
...
def main():
    ...
    user=pd.DataFrame(rows).merge(responses,on=["participant","filename"],how="left",validate="one_to_one")
    ...
    summary=pd.DataFrame({"metric":["Participants included","Images included","Usable participant-image observations",
        ...
        user["emotion_match_original_vs_response"].mean(),"Recoded as Neutral","Recoded as 3",str(HEATMAP_DIR)]})
