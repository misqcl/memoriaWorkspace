...
GENERAL_HEATMAP_DIR = ANALYSIS_DIR / "image_heatmaps" / "raw"
...
OUTPUT_DIR = ANALYSIS_DIR / "heatmap_similarity_extremes_vs_deepgaze"
...
def create_participant_heatmap(
    ...
    Coordinate conversion is vectorized because this function is called many
    times during the participant-by-image scan.
    """
    height, width = shape
    ...
    if right <= left or bottom <= top or fix_df.empty:
        return heatmap, 0

    sx = pd.to_numeric(fix_df["norm_pos_x"], errors="coerce").to_numpy(dtype=float)
    sy = pd.to_numeric(fix_df["norm_pos_y"], errors="coerce").to_numpy(dtype=float)
    ...
    x = np.rint(ix[valid] * (width - 1)).astype(np.intp)
    y = np.rint(iy[valid] * (height - 1)).astype(np.intp)
    np.add.at(heatmap, (y, x), 1.0)
...
def scan_participant_heatmaps(image_info: pd.DataFrame) -> pd.DataFrame:
    ...
    usable_raw = usable_raw.dropna(axis=1, how="all").reset_index(drop=True)
    ...
    usable = usable_raw.loc[:, participant_columns].copy()

    # Load each participant file only once and split it by image filename.
    # This avoids repeatedly filtering the full CSV for every image.
    participant_fixations: dict[str, dict[str, pd.DataFrame]] = {}
    fixation_paths: dict[str, Path] = {}

    print("Loading and indexing participant fixation files...")
    for participant in participant_columns:
        participant_name = str(participant)
        fixation_path = RESULTS_DIR / participant_name / "valid_fixations.csv"
        ...
        if not required_columns.issubset(all_fixations.columns):
            warnings.warn(
                f"Skipping {participant_name}: missing columns in {fixation_path}"
            )
            continue

        all_fixations = all_fixations.loc[
            :,
            ["filename", "norm_pos_x", "norm_pos_y"],
        ].copy()
        all_fixations["filename"] = all_fixations["filename"].astype(str)

        participant_fixations[participant_name] = {
            filename: group
            for filename, group in all_fixations.groupby("filename", sort=False)
        }
        fixation_paths[participant_name] = fixation_path

    if not participant_fixations:
        raise RuntimeError(
            "No valid participant fixation files were found under results/U*/valid_fixations.csv."
        )

    # Count only QA-valid participant-image combinations for an accurate ETA.
    jobs: list[tuple[int, pd.Series, str]] = []
    for image_position, (_, image_row) in enumerate(image_info.iterrows()):
        trial = int(image_row["trial_index"])
        qa_row = trial - 1
        if qa_row < 0 or qa_row >= len(usable):
            continue

        for participant in participant_columns:
            participant_name = str(participant)
            if participant_name not in participant_fixations:
                continue

            value = usable.iloc[qa_row][participant]
            if pd.isna(value):
                continue

            try:
                is_usable = int(float(value)) == 1
            except (TypeError, ValueError):
                is_usable = str(value).strip().lower() in {
                    "true", "yes", "usable"
                }

            if is_usable:
                jobs.append((image_position, image_row, participant_name))

    print(f"QA-valid participant-image jobs: {len(jobs)}")
    if tqdm is None:
        print(
            "Tip: install tqdm for a live progress bar and ETA: "
        ...
        job_iterator = tqdm(
            jobs,
            total=len(jobs),
            desc="Evaluating participant heatmaps",
            unit="comparison",
            dynamic_ncols=True,
        )

    # Cache image-level arrays. Each DeepGaze and reference heatmap is loaded
    # and resized once per image, then reused across all participants.
    image_cache: dict[
        tuple[int, str],
        tuple[tuple[int, int], np.ndarray, Path],
    ] = {}

    for job_number, (image_position, image_row, participant) in enumerate(
        job_iterator,
        start=1,
    ):
        trial = int(image_row["trial_index"])
        ...
        if tqdm is not None:
            job_iterator.set_postfix(
                participant=participant,
                image=filename,
                refresh=False,
            )
        elif job_number == 1 or job_number % 50 == 0 or job_number == len(jobs):
            print(
                f"[{job_number}/{len(jobs)}] "
                f"participant={participant}, image={filename}",
                flush=True,
            )

        cache_key = (trial, filename)
        ...
        if cached is None:
            ...
            reference_shape = np.load(
                general_heatmap_path,
                mmap_mode="r",
            ).shape
            deepgaze = resize_to_shape(
                np.load(deepgaze_path),
                reference_shape,
            )
            # Normalizing once here saves another full-array normalization for
            # every participant. similarity_metrics accepts normalized inputs.
            deepgaze = normalize_prob(deepgaze)
            cached = (reference_shape, deepgaze, deepgaze_path)
            image_cache[cache_key] = cached

        reference_shape, deepgaze, deepgaze_path = cached

        image_fixations = participant_fixations[participant].get(filename)
        if image_fixations is None or image_fixations.empty:
            continue
        ...
        rows.append({
            ...
            "sim": sim,
            "cc": cc,
            "empirical_path": str(fixation_paths[participant]),
            "deepgaze_path": str(deepgaze_path),
        })
    ...
    if result.empty:
        raise RuntimeError(
            "No participant heatmaps were reconstructed. "
            "Check results/U*/valid_fixations.csv and the QA file."
        )
    return add_combined_rank_score(result)
