...
OUTPUT_DIR = ANALYSIS_DIR / "aoi_analysis_EOCR"
...
# Accepted values in verification_status.
VALID_AOI_STATUSES = {"accepted", "manual"}

# If the generated AOI file still has blank verification_status,
# keep those rows. Set False after explicit manual verification if desired.
ALLOW_BLANK_VERIFICATION_STATUS = True

# Overlapping boxes of the same AOI type are treated as one semantic region.
# A fixation inside 2 text boxes still counts once as a "text fixation".
AOI_TYPES = ("face", "text")
...
# Option B override: NR values become missing and are excluded.
...
def union_area_rectangles(rectangles):
    ...
    Avoids double-counting overlapping OCR boxes.
    """
    rects = []
...
def build_image_metadata(aoi):
    ...
    # Include images with zero detected semantic AOIs using the image AOI file.
    if IMAGE_AOI_FILE.exists():
        ...
        # Fill dimensions from AOI rows by filename.
...
def load_response_emotions(participants):
    """
    Maps participant + filename -> reported emotion.
    Uses Summary.xlsx to map U1... to participant filenames, following the existing thesis scripts.
    """
...
def main():
    ...
    for p_idx, participant in enumerate(participants, start=1):
        ...
        for _, img in image_meta.iterrows():
            ...
            # Trial onset for TTFF.
            trial_start = img_fix["start_timestamp"].min()

            # Total trial denominators.
            ...
            for aoi_type in AOI_TYPES:
                ...
                # TTFF in milliseconds from first fixation onset in the trial to first AOI fixation onset.
                # NaN means the AOI was never fixated.
                ttff_ms = (
    ...
    valid_pairs = pd.read_csv(VALID_PAIRS_FILE, usecols=["participant", "filename"]).drop_duplicates()
    metrics = metrics.merge(valid_pairs, on=["participant", "filename"], how="inner", validate="many_to_one")
    ...
    # Thesis association: reported emotion × face presence.
    # Each participant-image response is an observation.
    # ======================================================
